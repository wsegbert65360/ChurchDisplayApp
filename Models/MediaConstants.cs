...
public static class MediaConstants
{
    ...
    {
        // ⚡ Bolt: Zero-allocation string matching using ReadOnlySpan and OrdinalIgnoreCase
        // Avoids string allocation from .ToLower() and enumerator allocation from LINQ .Contains()
        var ext = System.IO.Path.GetExtension(filePath.AsSpan());
        foreach (var e in ImageExtensions)
        {
            if (ext.Equals(e.AsSpan(), StringComparison.OrdinalIgnoreCase))
                return true;
        }
        return false;
    }
    ...
    {
        // ⚡ Bolt: Zero-allocation string matching
        var ext = System.IO.Path.GetExtension(filePath.AsSpan());
